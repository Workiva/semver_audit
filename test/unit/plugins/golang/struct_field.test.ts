...
test("adding a json struct tag with a different name as the field is a major", () =>
  expect(
    ...
    Semver.major(
      `Changing the name of the json serialized key is a major (Foo's serialization changed from 'Foo' to 'Bar')`,
    ),
  ]));
...
test("changing the name of a json struct tag is a major", () =>
  expect(
    ...
    Semver.major(
      `Changing the name of the json serialized key is a major (AField's serialization changed from 'Foo' to 'Bar')`,
    ),
  ]));
...
test("removing a field from json serialization via struct tag is a major", () =>
  expect(
    ...
    Semver.major(`Removing a field from being serialized is a major (AField)`),
  ]));
...
test("adding a field back to json serialization via struct tag is a minor", () =>
  expect(
    ...
    Semver.minor(`Including a field in json serialization is a minor (AField)`),
  ]));
