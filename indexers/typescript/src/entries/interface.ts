...
export class InterfaceEntry extends Entry<ts.InterfaceDeclaration> {
  ...
  grammar() {
    ...
    return {
      name,
      members: membersForInterfaceDeclaration(this.declaration, this.typeChecker),
...
function membersForInterfaceDeclaration(declaration: ts.InterfaceDeclaration, checker: ts.TypeChecker): Record<string, any> {
  ...
  for (let member of declaration.members) {
    if (ts.isPropertySignature(member) && member.type) {
      ...
      console.warn(`Unsupported interface member type for '${declaration.name}': (${ts.SyntaxKind[member.kind]})`)
    }
  }

  let extendedInterfaces = (declaration.heritageClauses ?? [])
